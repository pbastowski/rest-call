import { options as ng2nOptions, injector } from 'angular2now';
...
export function restConfig(options) {
    angular.merge(restOptions, options);
}

export function RestCall(apiTemplate, _options) {
    ...
    return function (target, name, descriptor) {
        ...
        descriptor.value = function () {
            if (!$http) {
                $http = injector(['ng']).get('$http');
                $q = injector(['ng']).get('$q');
                $log = injector(['ng']).get('$log');
                console.log('@ RestCall: $http, $q, $log:', $http, $q, $log);
            }
